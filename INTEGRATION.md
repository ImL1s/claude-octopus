...
| **Integration** | OpenCLI becomes a new Octopus provider, giving AI workflows access to live web data and desktop app control |
...
The integration adds 8 new tools to the Octopus MCP Server:
...
| `opencli_multi_search` | Cross-platform search | Search Twitter + Reddit + HN at once |
| `opencli_multi_trending` | Cross-platform trending | Trending from all platforms at once |
...
│   └── mcp-server/
│       └── src/index.ts   — MCP tools (+ 8 new opencli_* tools)
│
└── OpenCLI (npm global)
...
| `mcp-server/src/index.ts` | **MODIFIED** — Added 8 OpenCLI MCP tools |
