...
version: 1.0.0
description: "Real-time web research via OpenCLI: search social platforms, fetch trending topics, and extract URL content."
triggers: ["what's trending", "search Twitter for", "latest on Reddit", "fetch HN frontpage", "explore this URL", "real-time data"]
---
...
### 3. Fetch — Run Any OpenCLI Command

Run any OpenCLI command for a platform (e.g., get user profile, read thread).

```bash
# Via MCP tool
opencli_fetch(platform: "twitter", command: "thread", args: ["1234567890"])

# Via bridge script
${CLAUDE_PLUGIN_ROOT}/scripts/opencli-bridge.sh fetch twitter thread 1234567890
```
...
For desktop control, see [`skill-opencli-desktop`](../skill-opencli-desktop/SKILL.md).
...
opencli_multi_search(query: "AI agents", platforms: ["twitter", "reddit", "hackernews"])
opencli_multi_trending(platforms: ["twitter", "hackernews"])
...
[Executes: opencli_explore(url: "https://example.com/deep-article")]
